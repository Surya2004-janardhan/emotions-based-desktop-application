    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="h-1.5 rounded-full bg-cherry-dark overflow-hidden">
          <div
            className="h-full rounded-full bg-gradient-to-r from-wattle-dark to-wattle transition-all duration-700"
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
...
export default function CognitiveInsights({ results }) {
  ...
  return (
    <div className="max-w-2xl mx-auto glass glow-border rounded-2xl p-6 space-y-5 animate-fade-up" style={{ animationDelay: '0.15s' }}>
      ...
      {results.reasoning && (
        <p className="text-xs text-text-secondary leading-relaxed pt-4" style={{ borderTop: '1px solid rgba(213,207,47,0.1)' }}>
          {results.reasoning}
        </p>
      )}
    </div>
